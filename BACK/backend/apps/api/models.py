...
# ML RELATED MODELS -----> deploymachinelearning.com
...
class TextSample(models.Model):
    ...
    author = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='text_samples')
    endpoint_name = models.CharField(max_length=100, blank=True, null=True)
    mlrequest = models.ForeignKey(MLRequest, on_delete=models.CASCADE, null=True, blank=True)
    
    def __str__(self):
        return f"Text: {self.text[:20]} - Language Detected: {self.language_detected}"
